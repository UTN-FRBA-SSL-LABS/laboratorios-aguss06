...
> R:Lo que hace carrito_init es inicializar carrito en 0(productos y valores), es importante llamarla antes de usar el carrito porque si no lo inicializa, el carrito no tiene estado valido y no se puede agregar ni tampoco contabilizar los productos.
...
> R:El test falla, el valor que esperaba era 700 pero el que obtuvo fue de 350
...
> R:El bug esta en la linea 24 y el cogigo dice  total += c->items[i].precio
...
> R:Si falla, el 5to carrito devuelve esperado 0 , obtenido 1 error en la (linea 66).
...
> R:El operador incorrecto es <= y el que deberia ser es <
...
> R:Si las lineas que tienen ##### son las lineas 29 y 30, el error es que nunca se llamo en la ejecucion de los test
